                        ...
                        match value {
                            Some(timestamp) => {
                                channel.send_string(Some(&json!({
                                    "type": "gst-event-ack",
                                    "timestamp": timestamp,
                                    "computed_latency_mid": SystemTime::now().duration_since(SystemTime::UNIX_EPOCH).unwrap().as_millis() as u64 - timestamp,
                            ...
                            None => gst_error!(CAT, "Could not get timestamp from `{}`", msg),
                        }
