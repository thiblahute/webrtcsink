...
fn create_navigation_event(sink: gst_video::Navigation, msg: &str) {
    let event: Result<gst_video::NavigationEvent, _> = serde_json::from_str(msg);

    if let Ok(event) = event {
        sink.send_event(event.build().structure().unwrap().to_owned());
    } else {
...
#[derive(Debug)]
pub(crate) struct NavigationEventHandler {
...
fn make_converter_for_video_caps(caps: &gst::Caps) -> Result<gst::Element, Error> {
    ...
    for feature in caps.features(0) {
        if feature.contains(CUDA_MEMORY_FEATURE) {
            ...
            return Ok(gst::parse_bin_from_description(
                "glupload ! glcolorconvert ! glcolorscale ! videorate drop-only=true",
                true,
            )?
            .upcast());
        }
...
fn setup_encoding(
    ...
    let conv_caps = if codec.is_video {
        let structure_builder = gst::Structure::builder("video/x-raw")
            .field("pixel-aspect-ratio", gst::Fraction::new(1, 1));

        let structure_builder = if codec.encoder.name() == "nvh264enc" {
...
impl NavigationEventHandler {
    pub fn new(
        ...
        let channel = webrtcbin.emit_by_name::<WebRTCDataChannel>(
            "create-data-channel",
            &[&"input", &gst::Structure::new_empty("nothing")],
        );
        ...
        Self {
            message_sig: channel.connect("on-message-string", false, move |values| {
                if let Some(element) = weak_element.upgrade() {
                    create_navigation_event(element.upcast(), values[1].get::<&str>().unwrap());
                }
...
impl WebRTCSink {
    ...
    async fn run_discovery_pipeline(
        ...
        gst::Element::link_many(elements_slice).unwrap();
...
impl NavigationImpl for WebRTCSink {
    fn send_event(&self, _imp: &Self::Type, event_def: gst::Structure) {
        ...
        state
            ...
            .for_each(|(_, stream)| {
                if stream.sink_pad.name().starts_with("video_") {
                    gst_log!(CAT, "Navigating to: {:?}", event_def);
                    // FIXME: Handle multi tracks.
                    if !stream.sink_pad.push_event(gst::event::Navigation::new(event_def.clone())) {
                        gst_info!(CAT, "Could not send event: {:?}", event_def);
                    }
                }
            });
    }
