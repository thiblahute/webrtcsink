...
impl Detector {
    ...
    fn evict_old_received_packets(&mut self) {
        let last_arrival = self.last_received_packets.back().unwrap().arrival;

        while last_arrival - self.oldest_packet_in_window_ts() > *PACKETS_RECEIVED_WINDOW {
            self.last_received_packets.pop_front();
        }
    }
    ...
    fn effective_bitrate(&self) -> Bitrate {
        ...
        let duration = self.last_received_packets.back().unwrap().arrival
            - self.last_received_packets.front().unwrap().arrival;
        let bits = self
            .last_received_packets
            .iter()
            .map(|p| p.size as f64)
        ...
        if self.ema.estimate_is_close(effective_bitrate) {
            ...
            let increase = f64::max(
                ...
                f64::min(
                    ...
                    // FIXME: Until we implement "stuffing" which should ensure that the effective bitrate is
                    // not < target bitrate, increase by some arbitrary number minimum (160.bits)
                    f64::max(
