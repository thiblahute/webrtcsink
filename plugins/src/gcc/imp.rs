...
use std::{collections::VecDeque, fmt, fmt::Debug, mem, sync::Mutex, time};
...
impl Detector {
    ...
    fn kalman_estimate(&mut self, prev_group: &PacketGroup, group: &PacketGroup) {
        ...
        // FIXME: This doesn't exactly follows the spec as we should compute and
        // use f_max here.
...
impl ExponentialMovingAverage {
    fn update<T: Into<f64>>(&mut self, value: T) {
        if self.average == 0.0 {
            ...
            let avg_diff = value.into() - self.average;

            self.average += MOVING_AVERAGE_SMOOTHING_FACTOR * avg_diff;
...
struct State {
    ...
    periodic_push: Option<gst::PeriodicClockId>,
...
impl State {
    ...
    fn loss_control(&mut self, bwe: &super::BandwidthEstimator) -> bool {
            ...
            && (now - self.last_decrease_on_loss) > *LOSS_UPDATE_INTERVAL
        {
            ...
            self.last_control_op = BandwidthEstimationOp::Decrease {
        ...
        {
            self.last_control_op = BandwidthEstimationOp::Increase;
...
impl BandwidthEstimator {
    fn push_list(&self, list: gst::BufferList) {
    ...
    fn start_task(&self, bwe: &super::BandwidthEstimator) -> Result<(), glib::BoolError> {
        let weak_controller = bwe.downgrade();
        let weak_pad = self.srcpad.downgrade();
        let clock = gst::SystemClock::obtain();

        bwe.imp().state.lock().unwrap().periodic_push = Some(clock.new_periodic_id(
        ...
        self.srcpad.start_task(move || {
            ...
            let bwe = weak_controller
                .upgrade()
                .expect("bwe destroyed while its srcpad task is still running?");

            let lock_state = || bwe.imp().state.lock().unwrap();

            let periodic_id = match lock_state().periodic_push.take() {
                Some(id) => id,
                _ => {
                    gst::info!(CAT, "Peridic pushes are stop, pausing task");
                    return pause();
                }
            };

            if let (Err(err), _) = periodic_id.wait() {
                match err {
            ...
            let list = {
                ...
                state.periodic_push = Some(periodic_id);
                state.create_buffer_list(&bwe)
            };

            if list.len() > 0 {
                let _ = bwe.imp().push_list(list);
            }
        })?;
...
impl ObjectSubclass for BandwidthEstimator {
    const NAME: &'static str = "RTPGCCBwE";
...
impl Default for State {
    fn default() -> Self {
        Self {
            ...
            last_control_op: BandwidthEstimationOp::Increase,
